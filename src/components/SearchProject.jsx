...
const SearchProject = () => {
    ...
    const projectFiltered = searchProject === '' ? [] : projects.filter(project => project.name.toLowerCase().includes(searchProject.toLowerCase()));

    return (
        <Transition.Root show={search} as={Fragment} afterLeave={() => setSearchProject('')}>
                        ...
                        {projectFiltered.length > 0 && (
                            <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                                {projectFiltered.map(project => (
                                    <Combobox.Option key={project._id} value={project}
